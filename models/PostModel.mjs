...
const PostModel = {
    ...
    searchCategory: async (catData) => {
        ...
        try {
            ...
            if (subcategory != undefined || subcategory != null) {
                const searchSubCatSlug = await categoryModel.getSubCategoryBySlug(subcategory);
                const subCatname = searchSubCatSlug[0].subcat_name;
                dataCountQuery += ` and (subcat_slug = '${subcategory}' or meta_keywords like '%${subCatname}%') `
                dataQuery += ` and (subcat_slug = '${subcategory}' or meta_keywords like '%${subCatname}%') `
            }
            dataQuery += ` ORDER BY post_date DESC LIMIT ? OFFSET ? `;
            const [totalResult] = await pool.query(dataCountQuery, [category]);
            const totalItems = totalResult[0].total;
            
            // if no post found with category orsubcategory
            if(totalItems == 0){
    ...
    searchSubCategory: async (subCat) => {
        ...
        try {
            ...
            const scQuery = `select id ,post_title ,post_name ,SUBSTRING_INDEX(REGEXP_REPLACE(REPLACE(REPLACE(post_content, '\\r|\\n', ''), '\\t', ''), '<[^>]+>', ''), ' ', 30) AS p_content,post_author ,banner_img,post_date ,cat_slug,meta_keywords from p_data_view where cat_slug = ? and (subcat_slug = ? or meta_keywords like ? ) and post_status ='publish' ORDER BY post_date DESC LIMIT 7`;
    ...
    asideCategory: async (category) => {
        ...
        try {
            const asideQuery = `select id ,post_title ,post_name ,meta_description as post_content ,post_author ,banner_img,post_date,cat_slug from p_data_view where cat_slug = ? and post_status ='publish' ORDER BY id DESC LIMIT 5`
    ...
    latestNewsPost: async () => {
        try {
            const latestNewsQuery = `select id,post_author,post_date,banner_img,post_title,meta_description AS post_content,cat_slug,post_name from p_data_view WHERE post_status = 'publish' and cat_slug ='news' ORDER BY id DESC limit 3;`;
    ...
    latestPost: async () => {
        try {
            const latestQuery = `select id,post_author,post_date,banner_img,post_title,meta_description AS post_content,cat_slug,post_name from p_data_view WHERE post_status = 'publish' and cat_name NOT IN ('news','podcasts') ORDER BY id DESC limit 4;`;
    ...
    popularPost: async () => {
        try {
            const latestQuery = `SELECT p_data_view.id, COUNT(post_view_count.id) as post_count,post_author,post_date,banner_img,post_title,meta_description AS post_content,cat_slug,post_name FROM post_view_count LEFT JOIN p_data_view ON p_data_view.id = post_view_count.post_id WHERE p_data_view.post_status = "publish" GROUP BY post_id ORDER BY post_count DESC LIMIT 4`;
            const [postData] = await pool.query(latestQuery);
            return (postData)
        }
        catch (error) {
    ...
    quickBytePost: async () => {
        try {
            const quickByteQuery = `select id,post_author,post_date,banner_img,post_title,meta_description AS post_content ,cat_slug,post_name from p_data_view WHERE post_status = 'publish' and cat_slug ='quick-bytes' ORDER BY id DESC limit 6;`;
    ...
    articlePost: async () => {
        try {
            const articleQuery = `select id,post_author,post_date,banner_img,post_title,meta_description AS post_content ,cat_slug,post_name from p_data_view WHERE post_status = 'publish' and cat_slug ='featured' ORDER BY id DESC limit 6;`;
    ...
    interviewPost: async () => {
        try {
            const articleQuery = `select id,post_author,post_date,banner_img,post_title,meta_description AS post_content,cat_slug,post_name from p_data_view WHERE post_status = 'publish' and cat_slug ='interview' ORDER BY id DESC limit 10;`;
            const [postData] = await pool.query(articleQuery);
            return (postData)
        }
        catch (error) {
    ...
    podcastPost: async () => {
        try {
            const podcastQuery = `select id,post_author,post_date,banner_img,post_title,meta_description AS post_content ,cat_slug,post_name from p_data_view WHERE post_status = 'publish' and cat_slug ='podcasts' ORDER BY id DESC limit 1;`;
    ...
    latestPage : async(req,res)=>{
        try {
            ...
            const latestQuery = `select id,post_author,post_date,banner_img,post_title,meta_description AS post_content ,cat_slug,post_name from p_data_view WHERE post_status = 'publish' and cat_slug = ? ORDER BY id DESC limit ?;`;
